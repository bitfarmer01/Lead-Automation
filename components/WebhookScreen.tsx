  ...
  return (
    <div className="space-y-8 relative">
       <button 
          ...
          className="absolute -top-8 -left-4 flex items-center gap-2 text-slate-400 hover:text-yellow-400 transition-colors"
       >
      ...
      <section className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 backdrop-blur-sm">
        <h2 className="text-xl font-semibold mb-4 flex items-center text-slate-100">
          <span className="bg-yellow-600 text-slate-900 rounded-full h-8 w-8 flex items-center justify-center mr-3 font-bold">2</span>
          Your Webhook URL
        </h2>
        <div className="space-y-4">
          <div className="flex gap-2 items-center">
            <select 
              ...
              className="bg-slate-900 border border-slate-600 rounded-md p-3 focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none transition duration-200"
            >
              <option value="">Load a saved URL...</option>
              {savedWebhooks.map(w => <option key={w.name} value={w.name}>{w.name}</option>)}
            </select>
            {selectedWebhookName && (
              <button onClick={handleDeleteSelectedWebhook} className="p-3 text-slate-400 hover:text-red-400 transition" title="Delete selected webhook">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-600 rounded-md p-3 focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none transition duration-200"
            />
            <button onClick={handleOpenSaveModal} className="p-3 bg-slate-700 text-yellow-400 rounded-md hover:bg-slate-600 transition" title="Save this URL">
      ...
      <section className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 backdrop-blur-sm">
        <h2 className="text-xl font-semibold mb-4 flex items-center text-slate-100">
          <span className="bg-yellow-600 text-slate-900 rounded-full h-8 w-8 flex items-center justify-center mr-3 font-bold">3</span>
          Review Payload
        </h2>
      ...
      {showConfirmation && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" aria-modal="true" role="dialog">
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-w-lg w-full p-6 text-slate-200">
              <h2 className="text-2xl font-bold mb-4 text-yellow-400">Confirm Payload</h2>
              <p className="mb-2 text-slate-400">You are about to send <span className="font-bold text-yellow-500">{entries.length}</span> {entries.length === 1 ? 'request' : 'requests'} to:</p>
              <p className="font-mono bg-slate-900 p-2 rounded-md text-sm text-slate-300 break-all mb-6">{webhookUrl}</p>
              <div className="space-y-3 max-h-60 overflow-y-auto pr-2 mb-6">
                {entries.map((entry, index) => (
                   <div key={entry.id} className="p-3 bg-slate-900/50 rounded-md border border-slate-700/50 text-sm">
                     <p className="font-bold text-slate-300 mb-1">Request #{index+1}</p>
                     <p className="text-slate-400">Find <span className="text-yellow-400 font-semibold">'{entry.jobRole}'</span> jobs in <span className="text-yellow-400 font-semibold">'{entry.location}'</span> for <span className="text-yellow-400 font-semibold">{entry.experienceLevel.replace(/_/g, ' ')}</span> level.</p>
                   </div>
                ))}
              </div>
              <div className="flex justify-end gap-4">
                <button onClick={() => setShowConfirmation(false)} className="px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-500 transition text-slate-200 font-semibold">Cancel</button>
        ...
        {showSaveModal && (
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" aria-modal="true" role="dialog">
                <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-w-md w-full p-6 text-slate-200">
                    <h2 className="text-2xl font-bold mb-4 text-yellow-400">Save Webhook URL</h2>
                    <p className="font-mono bg-slate-900 p-2 rounded-md text-sm text-slate-300 break-all mb-4">{webhookUrl}</p>
                    <label htmlFor="webhookName" className="block text-sm font-medium text-slate-400 mb-2">Save as:</label>
                    <input id="webhookName" type="text" value={newWebhookName} onChange={(e) => setNewWebhookName(e.target.value)} placeholder="e.g., Staging API" className="w-full bg-slate-900 border border-slate-600 rounded-md p-3 focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none transition duration-200" />
